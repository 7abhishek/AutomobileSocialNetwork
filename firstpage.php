...
function loadchat()
{
	...
	var frn=document.getElementById("frn1").value;
 
		 $.ajax({
			type:"GET",
			url:"chatfiles/kumarkashif.php?p="+frn,
...
if(mysql_num_rows($re))
{
		...
		echo " $('#fpil').css('background-color','YELLOW',1000);";
}
?>
...
<div id="chat">
<span class="label label-info span12"><h5 align="center">CHAT</h5></span><br>
<?php
...
$row=mysql_query("(select f_email as mail from friends where u_email='$sesid' and request='accepted') union(
select u_email as mail from friends where f_email='$sesid' and request='accepted')")or die(mysql_error());
echo "<select name='friend' size='1' id='frn1' class='nav navbar' onchange='loadchat()'>";
echo "<option>SELECT A FRIEND</option>";
...
while($data=mysql_fetch_array($row))
{
	...
	{
		$mai=$data['mail'];
		$rr=mysql_query("select name from user where email='$mai'") or die(mysql_error());
...
echo "</select>";
echo "<div class='' id='msg'>
</div><br>
<div class='textarea'>
<textarea  cols='30' rows='5' id='textarea1' placeholder='write here to chat...' required='required' name='meassage' class='text-success'>
</textarea><br><button type='submit' class='btn' value='done' onclick=' loadc()'>SEND</button>
</div>
";
?>
</div><br><br>
&nbsp;
<div id="distributor">
<span class="label label-important span12"><h5 align="center">DISTRIBUTOR</h5></span><br>
